...
const CaseForm = ({ case: caseData, onSubmit, onCancel, currentUser }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Charger les clients avec leur code_client
      const { data: clientsData, error: clientsError } = await supabase
        .from('clients')
        .select('id, client_code, name, type, first_name, last_name, company')
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {caseData && caseData.id_dossier && (
            <div>
              ...
              <input
                type="text"
                value={caseData.id_dossier}
                disabled
                className="w-full px-4 py-3 bg-slate-700/30 border border-slate-600 rounded-lg text-slate-400 cursor-not-allowed"
              />
              <p className="text-xs text-slate-500 mt-1">Identifiant numérique généré automatiquement (lecture seule)</p>
            </div>
          )}
          ...
          {/* 4. Client (select avec code_client + nom) */}
          <div>
            ...
            >
              ...
              {clients.map(client => (
                <option key={client.id} value={client.id}>
                  {client.client_code ? `${client.client_code} - ` : ''}{client.name}
                </option>
              ))}
            </select>
          ...
          <div>
            ...
            <div className="flex gap-4">
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                  type="radio"
                  name="qualite_du_client"
                  value="personne_physique"
                  checked={formData.qualite_du_client === 'personne_physique'}
                  onChange={handleChange}
                  className="w-4 h-4 text-indigo-600 focus:ring-indigo-500"
                />
                <span className="text-slate-300">Personne physique</span>
              </label>
              <label className="flex items-center space-x-2 cursor-pointer">
                <input
                  type="radio"
                  ...
                  checked={formData.qualite_du_client === 'personne_morale'}
                  onChange={handleChange}
                  className="w-4 h-4 text-indigo-600 focus:ring-indigo-500"
                />
                <span className="text-slate-300">Personne morale</span>
              </label>
            </div>
          </div>
          ...
          <div>
            ...
            >
              ...
              <option value="Rédaction">Rédaction d'actes</option>
          ...
          <div>
            ...
            <input
              ...
              onChange={handleChange}
              className="w-full px-4 py-3 bg-slate-700/50 border border-slate-600 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              placeholder="Ex: Litige commercial relatif à un contrat de prestation"
            />
            <p className="text-xs text-slate-500 mt-1">Objet juridique du dossier (différent de la description)</p>
          </div>
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-2">
              🔒 Seuls les utilisateurs sélectionnés et les administrateurs peuvent consulter le contenu complet.
            </p>
          </div>
